...
if ($password1=='' && $filename=="") {
	...
	echo "<script>alert('Profil Berhasil Diubah'); window.location.href = 'profil.php';</script>";
}
...
elseif($password1==''){
	...
	if(in_array($ext, $allowed) ){
		$datalama = mysqli_query($koneksi,"select * from user where id_user='$id_user'");
			...
			unlink("../img_admin/".$nama_data_lama);

			 move_uploaded_file($_FILES['file']['tmp_name'],'../img_admin/'.$rand. '_'.$filename);
